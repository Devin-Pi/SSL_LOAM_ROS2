...
void MapOptimizationClass::addPoseToGraph(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pointcloud_edge_in,
                                          const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pointcloud_surf_in,
...
bool MapOptimizationClass::optimizeGraph(int matched_id, int current_id)
{
    ...
    for (int i = matched_id + 1; i <= current_id; i++)
    {
        ...
        graph.push_back(gtsam::BetweenFactor<gtsam::Pose3>(gtsam::Symbol('x', i - matched_id),
                                                           gtsam::Symbol('x', i + 1 - matched_id),
                                                           pose_optimized_arr[i - 1].between(pose_optimized_arr[i]), odomModel));
    }
    graph.push_back(gtsam::BetweenFactor<gtsam::Pose3>(gtsam::Symbol('x', 1),
                                                       gtsam::Symbol('x', current_id - matched_id + 1),
                                                       eigenToPose3(transform), loopModel));
...
bool MapOptimizationClass::geometryConsistencyVerification(int matched_id, int current_id, Eigen::Isometry3d &transform)
{
    ...
    {
        RCLCPP_WARN(rclcpp::get_logger("map_optimization"),
                    "loop rejected due to geometry verification current_id%d matched_id %d, score: %f solution",
                    current_id, matched_id, match_score);
        RCLCPP_WARN(rclcpp::get_logger("map_optimization"),
                    "No loop closure identified, the map will be generated without global optimization. if you want to force a loop closure happen, change your loop closure threshold higher");
        return false;
    }
}

double MapOptimizationClass::estimateOdom(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pc_source_edge,
                                          const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pc_source_surf,
                                          const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pc_target_edge,
                                          const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pc_target_surf,
                                          Eigen::Isometry3d &transform)
{
    ...
    for (int opti_counter = 0; opti_counter < 10; opti_counter++)
    {
        ...
        if (corner_num < 20)
        {
            RCLCPP_INFO(rclcpp::get_logger("map_optimization"), "not enough corresponding points");
            return 300.0;
        }
